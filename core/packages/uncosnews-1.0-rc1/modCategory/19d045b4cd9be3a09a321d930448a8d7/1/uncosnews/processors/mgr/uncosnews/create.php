...
if (empty($scriptProperties['title'])) {
    $modx->error->addField('title',$modx->lexicon('uncosnews.err_ns_title'));
} else {
    $alreadyExists = $modx->getObject('UncosNewsNews',array('title' => $scriptProperties['title']));
    if ($alreadyExists) {
        $modx->error->addField('title',$modx->lexicon('uncosnews.err_ae_title'));
    }
}
if (empty($scriptProperties['text']))  $modx->error->addField('text',$modx->lexicon('uncosnews.err_ns_text'));
 
if ($modx->error->hasError()) { return $modx->error->failure(); }
 
$newsItem = $modx->newObject('UncosNewsNews');
...
if ($newsItem->save() == false) {
    return $modx->error->failure($modx->lexicon('uncosnews.err_save'));
}
