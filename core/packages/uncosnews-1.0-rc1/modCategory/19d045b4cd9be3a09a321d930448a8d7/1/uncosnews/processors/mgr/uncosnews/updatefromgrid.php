...
/* get obj */
if (empty($_DATA['id'])) return $modx->error->failure($modx->lexicon('uncosnews.err_ns'));
$newsItem = $modx->getObject('UncosNewsNews',$_DATA['id']);
if (empty($newsItem)) return $modx->error->failure($modx->lexicon('uncosnews.err_nf'));
$newsItem->set('editedby', $modx->user->get('id'));
$newsItem->set('editedon', strftime("%Y-%m-%d %H:%M:%S"));
